          ...
          <table className="min-w-full divide-y divide-slate-200/80">
            ...
            <tbody className="divide-y divide-slate-200/80">
              {(data?.items ?? []).map((asset) => (
                <tr
                  key={asset.id}
                  className="cursor-pointer text-sm transition hover:bg-brand-50/50"
                  onClick={() => navigate(`/assets/${asset.id}`)}
                >
                  <td className="px-6 py-4">
                    <div className="space-y-1">
                      <p className="font-semibold text-slate-900">{asset.asset_tag}</p>
                  ...
                  <td className="px-6 py-4">
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-slate-500">
                        {asset.category.name}
                        {asset.assigned_user ? ` · ${asset.assigned_user.full_name}` : ""}
                      </p>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4 text-slate-700">
                    <div className="space-y-1.5">
                      ...
                      {!asset.cost_center && !asset.warranty_expiry_date && !asset.expected_end_of_life_date && "-"}
                    </div>
                  </td>
                </tr>
              ))}
